...
schema = DataFrameSchema(
	columns={
		...
		"gs-sample_name": Column(
			...
			coerce=False,
			required=True,
			description="Identifier name used for GISAID. Max length is 50 characters. This field is the same as \"pox_sequence_name\" in GISAID's metadata template.",
			title="sample name",
		),
		"gs-pox_passage": Column(
			dtype="object",
			checks=[
				Check.str_matches(r"^(?!\s*$).+"),
			],
		...
		"gs-pox_sampling_strategy": Column(
			dtype="object",
			checks=None,
			nullable=True,
			unique=False,
		...
		"gs-pox_provider_sample_id": Column(
			dtype="object",
			checks=None,
			nullable=True,
			unique=False,
     ...
     title="seqsender GISAID MPOX schema",
     description="Schema validation for GISAID monkeypox database.",
)
